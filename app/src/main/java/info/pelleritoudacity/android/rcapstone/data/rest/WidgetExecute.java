...
public class WidgetExecute {
    ...
    public WidgetExecute(OnRestCallBack callBack, Context context, String category) {
    ...
    public void getData() {
        ...
        if (isAuthenticated) {
            ...
            sApi.getWidgetAuth(TextUtil.authCode(PermissionUtil.getToken(mContext)), mCategory, fieldMap)
            ...
            sApi.getWidget(mCategory, fieldMap)
                    .enqueue(new Callback<T3>() {
                        @Override
                        public void onResponse(@NonNull Call<T3> call, @NonNull Response<T3> response) {
                            mCallBack.success(response.body(), response.code());
    ...
    public void getDataList() {
        ...
        if (isAuthenticated) {
            sApi.getSortWidgetAuth(TextUtil.authCode(PermissionUtil.getToken(mContext)),
                    mCategory,
